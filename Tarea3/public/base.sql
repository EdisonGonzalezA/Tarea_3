...
CREATE TABLE IF NOT EXISTS `Sexto`.`Proveedores` (
  ...
  `Contacto_Empresa` VARCHAR(17) NOT NULL COMMENT 'Campo para lamacenar el nombre del empleado de la empresa para contactarse',
  `Telefono_Empresa` VARCHAR(17) NOT NULL COMMENT 'Campo para almacenar el numero de telefono del contacto de la empresa',
...
CREATE TABLE IF NOT EXISTS `Sexto`.`Unidad_Medida` (
  ...
  `Detalle` TEXT NULL,
  `Tipo` INT NULL COMMENT '1 = Unidad de Medida Ej: Gramo, Litros, Kilos\n0 = Presentacion Ej: Caja, Unidad, Docena, Sixpack',
...
CREATE TABLE IF NOT EXISTS `Sexto`.`IVA` (
  ...
  `Detalle` VARCHAR(45) NULL COMMENT '8%\n12%\n15%',
  `Estado` INT NULL COMMENT '1 = activo\n0 = inactivo',
  PRIMARY KEY (`idIVA`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Sexto`.`Productos` (
  ...
  `Graba_IVA` INT NULL COMMENT 'Campo para almacenar si el producto graba IVA\n1 = Graba IVA\n0 = No posee IVA',
  `Kardex_idKardex` INT NOT NULL,
  PRIMARY KEY (`idProductos`),
  INDEX `fk_Productos_Kardex1_idx` (`Kardex_idKardex` ASC) ,
  CONSTRAINT `fk_Productos_Kardex1`
